...
// ── Config — reads from .env file (never hardcode tokens) ───────────────────
const HUBSPOT_TOKEN = import.meta.env.VITE_HUBSPOT_TOKEN;
const NOTION_TOKEN  = import.meta.env.VITE_NOTION_TOKEN;
const NOTION_DB_ID  = import.meta.env.VITE_NOTION_DB_ID;

// All HubSpot & Notion calls route through Cloudflare Worker to bypass CORS
const PROXY = "https://venture-proxy.clashworldgame88.workers.dev";
...
// ── HubSpot API helpers (routed through Cloudflare proxy) ────────────────────
async function hsGet(path){
  const r = await fetch(`${PROXY}?target=hubspot&path=${encodeURIComponent(path)}`,{
...
// ── Notion API helpers (routed through Cloudflare proxy) ─────────────────────
async function notionGet(path){
  const r = await fetch(`${PROXY}?target=notion&path=${encodeURIComponent("/v1"+path)}`,{
